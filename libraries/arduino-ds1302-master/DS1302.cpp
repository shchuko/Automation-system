...
Time::Time(const uint8_t _yr, const uint8_t _mon, const uint8_t _date,
           const uint8_t _hr, const uint8_t _min, const uint8_t _sec,
           const uint8_t _day) {
  yr   = _yr;
  mon  = _mon;
  date = _date;
...
DS1302::DS1302(const uint8_t _ce_pin, const uint8_t _io_pin,
...
uint8_t DS1302::readRegister(const uint8_t reg) {
  const SPISession s(ce_pin, io_pin, sclk_pin);
